...
| **JS/TS/JSX/TSX** | ES6 / CommonJS imports, default/named exports, aliases. Component usages like `<UserList />` resolved as call edges. `tsconfig`/`jsconfig` paths. Relative React Router paths like `path: 'settings'` are supported. Remix/React Router route-module entry points like `loader` and `action` are recognized and excluded from dead code. | Dynamic imports (`import()`) or dynamic `require()` are not resolved. Dynamic member access (e.g. `obj[key]()`) cannot be resolved. |
| **Next.js** | App Router and Pages Router file conventions (page, layout, error, route, API files). Main default component exports (named or default) are recognized and excluded from dead code. | Dynamic imports or dynamic middleware routing are not supported. |
| **Svelte / SvelteKit** | Svelte `<script>` script blocks, template tag/expression refs, Kit file-convention routes (`+page.svelte`, `+server.ts`), and route handlers. GET/POST etc. endpoint functions in `+server.ts` files are recognized and excluded from dead code. | Complex reactive variables or store-based side-effects might not be modeled as call edges. |
...
| **Terraform / Terragrunt / OpenTofu** | Definitions for variables, locals, outputs, module calls, resources, data sources, providers, Terragrunt stacks and config layers (`internal/providers/hcl.go`). Edges for `var.x`/`local.x` (dir-scoped, Terraform's own rule), `module.N.out` → the target module's output, module-call arguments → target variables, `.tfvars` assignments, and Terragrunt `include`/`dependency`/`terraform.source`/`inputs` wiring. Local sources resolve relative paths, `${get_repo_root()}//...`, and `find_in_parent_folders()`. `.tofu`/`.tofuvars` are aliased to the HCL grammar. There is deliberately **no global by-name fallback** — a dangling `var.x` stays unresolved rather than linking to another module's same-named variable. | Registry/git module sources are external dependencies (surfaced by `deps`), not edges. Dynamic `for_each` module keys, `read_terragrunt_config` indirection, `generate`-block bodies, `TF_VAR_*` injection, and `terraform_remote_state` readers are not followed. Lock files are skipped by the graph (the `deps` tool reads them). |
