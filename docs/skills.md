...
|-------|------|---------------|
| **`codebase-walkthrough`** | Teaching the codebase phase-by-phase; the first-time durable guide; the interactive clickable-HTML map. | `recon`, `trace_flow`, `render_map`, `guide_*` |
| **`architecture-cartographer`** | Durable, *committable* diagram-as-code (Mermaid C4 / flowchart / ERD / dependency graph). | `recon`, `impact`, `render_map` (mermaid) |
| **`guide-maintainer`** | The git-SHA delta loop that keeps an existing cached guide current with HEAD. | `guide_read/write/delta`, `recon` |
| **`dependency-impact-analyzer`** | The per-change blast radius of one target ("what breaks if I change X"). | `impact`, `trace_flow` |
| **`test-gap-and-risk-auditor`** | A standing, whole-repo risk register: untested paths, fragile seams, silent assumptions. | `recon`, `trace_flow`, `impact` |

The deliberate split: walkthrough **teaches**; cartographer **draws** durable diagrams
(interactive HTML stays with walkthrough); guide-maintainer runs the **delta loop**;
...
(e.g. `codebase-walkthrough/references/{conversational,cached-guide,interactive-map,mcp-setup}.md`).
...
- **`name`** — lowercase letters/numbers/hyphens, ≤64 chars, no path separators or `..`
  (the installer refuses names that could escape the skills dir).
...
1. Create `internal/skills/assets/<name>/SKILL.md` with valid frontmatter (mind the
   plain-scalar rule above) and a thin body.
